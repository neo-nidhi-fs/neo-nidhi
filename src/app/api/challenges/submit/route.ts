import { NextResponse } from 'next/server';
import { dbConnect } from '@/lib/dbConnect';
import { Challenge } from '@/models/Challenge';
import { ChallengeParticipant } from '@/models/ChallengeParticipant';
import { QuizQuestion } from '@/models/QuizQuestion';

export async function POST(req: Request) {
  try {
    await dbConnect();
    const body = await req.json();
    const { userId, challengeId, answers } = body;

    if (!userId || !challengeId || !answers) {
      return NextResponse.json(
        { success: false, error: 'Missing required fields' },
        { status: 400 }
      );
    }

    // Get challenge
    const challenge = await Challenge.findById(challengeId);
    if (!challenge) {
      return NextResponse.json(
        { success: false, error: 'Challenge not found' },
        { status: 404 }
      );
    }

    // Get participant
    const participant = await ChallengeParticipant.findOne({
      userId,
      challengeId,
    });
    if (!participant) {
      return NextResponse.json(
        { success: false, error: 'User not registered for this challenge' },
        { status: 403 }
      );
    }

    // Get answered questions to verify
    const questionIds = answers.map(
      (a: { questionId: string }) => a.questionId
    );
    const questions = await QuizQuestion.find({ _id: { $in: questionIds } });

    let score = 0;
    let correctAnswers = 0;
    let totalPoints = 0;

    const answersWithResults = answers
      .map((answer: { questionId: string; selectedOption: number }) => {
        const question = questions.find(
          (q) => q._id.toString() === answer.questionId
        );
        if (!question) return null;

        const isCorrect = question.correctAnswer === answer.selectedOption;
        const points = isCorrect ? question.points : 0;

        if (isCorrect) {
          correctAnswers++;
          score += points;
        }
        totalPoints += question.points;

        return {
          questionId: question._id,
          selectedOption: answer.selectedOption,
          isCorrect,
        };
      })
      .filter(Boolean);

    // Update participant
    participant.score = score;
    participant.correctAnswers = correctAnswers;
    participant.totalQuestions = answers.length;
    participant.answers = answersWithResults;
    participant.status = 'completed';
    participant.completedAt = new Date();
    await participant.save();

    return NextResponse.json({
      success: true,
      data: {
        score,
        correctAnswers,
        totalQuestions: answers.length,
        totalPoints,
        percentage: Math.round((score / totalPoints) * 100),
      },
    });
  } catch (error: unknown) {
    return NextResponse.json(
      { success: false, error: (error as Error).message },
      { status: 500 }
    );
  }
}
