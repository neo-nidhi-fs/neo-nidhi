...
export default function ChallengePage() {
      ...
      try {
        ...
        if (data.success) {
          ...
          if (data.challenge.endDate) {
            const remaining = Math.max(0, Math.floor((new Date(data.challenge.endDate).getTime() - Date.now()) / 1000));
  ...
  const handleAnswer = (selectedOption: number) => {
    ...
    newAnswers[currentQuestion] = { questionId: questions[currentQuestion]._id, selectedOption };
  ...
  const handleSubmitChallenge = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/challenges/submit', {
        ...
        body: JSON.stringify({ userId: session.user.id, challengeId, answers }),
      });
      const data = await res.json();
      if (data.success) {
        setResult(data.data);
  ...
  if (!session) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-b from-slate-950 via-blue-950 to-slate-950">
        <p className="text-gray-100">Please log in</p>
      </div>
    );
  }
    ...
    <main className="bg-gradient-to-b from-slate-950 via-blue-950 to-slate-950 text-white min-h-screen py-12 px-6">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-200">Great job! Here&apos;s your result</p>
        </div>
            ...
            <CardContent className="pt-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="text-center">
                  ...
                  <p className="text-gray-400 mt-2">out of {result?.totalPoints || 0} points</p>
                </div>
                <div className="text-center">
                  <p className="text-gray-300 mb-2">Accuracy</p>
                  <p className="text-6xl font-black text-blue-400">{result.percentage}%</p>
        ...
        <div className="flex gap-4 flex-wrap justify-center">
          ...
          <Link href="/user/dashboard">
            <Button className="bg-slate-700 hover:bg-slate-600">Back to Dashboard</Button>
          </Link>
        </div>
      </div>
    </main>
