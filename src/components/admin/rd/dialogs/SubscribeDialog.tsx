...
export default function SubscribeDialog({ open, onClose, scheme, userId, onSave }: Props) {
  const [monthlyAmount, setMonthlyAmount] = useState(String(scheme.minMonthlyAmount));
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="w-[90vw] sm:w-full max-w-sm max-h-[90vh] overflow-y-auto p-4 sm:p-6 rounded-lg bg-slate-800 border-slate-700">
        <DialogHeader>
          <DialogTitle className="text-cyan-400">Subscribe to {scheme.name}</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-3">
          ...
          <div>
            <label className="block text-sm text-gray-300 mb-1">
              ...
              <span className="text-gray-500 ml-1">
                ...
                {scheme.maxMonthlyAmount != null ? ` · max ₹${scheme.maxMonthlyAmount}` : ''}
              </span>
            </label>
            <input
              className={inputCls}
          ...
          <div>
            <label className="block text-sm text-gray-300 mb-1">
              Mandate Day (1–28)
              <span className="text-gray-500 ml-1">day of month for auto-debit</span>
            </label>
            <input
              className={inputCls}
              type="number"
