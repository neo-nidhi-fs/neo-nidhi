...
export default function RDSubscriptionCard({ subscription: sub }: Props) {
  ...
  const progress = tenure && tenure > 0 ? Math.min((sub.installmentsPaid / tenure) * 100, 100) : 0;

  return (
    <div className="bg-slate-800/60 border border-slate-700 rounded-xl p-5 flex flex-col gap-3">
      <div className="flex items-start justify-between gap-2">
        <div>
          <h3 className="text-white font-semibold text-base">{schemeName(sub)}</h3>
          {schemeRate(sub) != null && (
            <p className="text-gray-400 text-xs mt-0.5">{schemeRate(sub)}% p.a.</p>
          )}
        </div>
        <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${STATUS_STYLES[sub.status] ?? ''}`}>
          {sub.status}
        </span>
      </div>
      ...
      {tenure != null && (
        <div>
          <div className="flex justify-between text-xs text-gray-400 mb-1">
            <span>{sub.installmentsPaid} of {tenure} installments</span>
      ...
      <div className="grid grid-cols-2 gap-2 text-sm">
        ...
        <div>
          ...
          <p className="text-white">{sub.mandateDay}{['st','nd','rd'][sub.mandateDay - 1] ?? 'th'} of month</p>
        </div>
        <div>
          <p className="text-gray-400 text-xs">Total Deposited</p>
          <p className="text-white">₹{sub.totalDebited.toLocaleString('en-IN')}</p>
        </div>
        <div>
          <p className="text-gray-400 text-xs">Accrued Interest</p>
          <p className="text-green-400">₹{(sub.accruedInterest ?? 0).toFixed(2)}</p>
        </div>
        ...
        {sub.maturityAmount != null && (
          <div className="col-span-2">
            ...
            <p className="text-green-400 font-semibold text-base">₹{sub.maturityAmount.toFixed(2)}</p>
          </div>
        )}
