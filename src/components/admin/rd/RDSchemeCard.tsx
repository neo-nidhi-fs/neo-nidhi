...
import type { IRDScheme, ICreateSubscriptionRequest } from '@/lib/services/rdNewService';
    ...
    <>
      <div className="bg-slate-800/60 border border-slate-700 rounded-xl p-5 flex flex-col gap-3 hover:border-blue-500/50 transition-colors">
        <div className="flex items-start justify-between gap-2">
          <div>
            <h3 className="text-white font-semibold text-base">{scheme.name}</h3>
            {scheme.description && (
              <p className="text-gray-400 text-xs mt-0.5">{scheme.description}</p>
            )}
          </div>
        ...
        <div className="grid grid-cols-2 gap-2 text-sm">
          ...
          <div>
            ...
            <p className="text-cyan-400 font-medium">₹{Math.round(estimatedMaturity).toLocaleString('en-IN')}</p>
          </div>
        </div>
